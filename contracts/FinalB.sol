...
contract FinalB{
    ...
    address Owner; //adminstrator who manages who can add challan etc.
    mapping(string=>Challan[]) ChallanHistory; //use to store all previous challans of citizens
    mapping(string=>Challan) ActiveChallan;  //use to store current active challans of citizen
    mapping(address=>bool) TP; //mapping for traffic police(TP) , who can add Challan
    VoilationRule[] RuleList; //list of traffic rules
...
////////////////////////////////////////////////////////////////functions related to admin
    //working
    function transferAdmin(address _temp) public{//working
    ...
    //working
    function addTrafficPolice(address _TP) public{//add traffic police who can add challans //woring
        require(msg.sender==Owner);
        TP[_TP]=true;
    }
    ...
    function addTrafficRule(string calldata _description,uint _Afine,uint _Bfine,uint _Cfine) public{ 
        require(msg.sender==Owner);
        VoilationRule memory temp=VoilationRule(_description,_Afine,_Bfine,_Cfine,true);
        RuleList.push(temp);
    }
    ...
    function addChallan(string memory _LicenseID,string memory _Name,string memory _CNIC,VehicleType _CarType,string memory _carPlate,uint[] memory _voilatedRules,string memory _timeStamp) public{
        ...
        Challan memory temp=Challan(_LicenseID,_Name,_CNIC,_CarType,_carPlate,_voilatedRules,_timeStamp,VoilationFee,true);
        ActiveChallan[_LicenseID]=temp;
        ChallanHistory[_LicenseID].push(temp);
